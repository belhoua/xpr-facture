...
function CommandDialog({
  ...
  return (
    <Dialog {...props}>
      ...
      >
        {/* En-tête a11y masqué : DialogTitle/Description DOIVENT vivre à
            l'intérieur de DialogContent pour que Radix les associe au dialogue. */}
        <DialogHeader className="sr-only">
          <DialogTitle>{title}</DialogTitle>
          <DialogDescription>{description}</DialogDescription>
        </DialogHeader>
        {/* `Command` (racine cmdk) fournit le store que consomment
            CommandInput/List/Item. Sans lui, les primitives lisent un store
            `undefined` → « Cannot read properties of undefined (reading
            'subscribe') » à l'ouverture. */}
